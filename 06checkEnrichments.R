...
# create gene universe for Fisher's test
...
# add dummy variable
fisher.genes[, same.efo := ifelse(efo.id.opentargets.degs == efo.id.stopgap, TRUE, FALSE)]

# plot adjusted p-value distributions
ggplot(fisher.genes, aes(x = same.efo, y = -log10(p.adjusted))) +
...
ggsave("../dat/fisher.genes.png", width = 20, height = 10, unit = "cm", dpi = 150)

# test if adjusted p-value  distributions significantly different with Mann-Whitney test (Wilcoxon rank sum test)
res <- wilcox.test(x = fisher.genes[same.efo == FALSE, -log10(p.adjusted)], y = fisher.genes[same.efo == TRUE, -log10(p.adjusted)])
print(res)
print(res$p.value)

# export
fwrite(fisher.genes[p.adjusted < 0.05, ], "../dat/fisher.genes.tsv", sep = "\t")
...
# add dummy variable
gsea.genes[, same.efo := ifelse(efo.id.opentargets.degs == efo.id.stopgap, TRUE, FALSE)]

# plot adjusted p-value distributions
ggplot(gsea.genes, aes(x = same.efo, y = -log10(p.adjusted))) +
    geom_boxplot(outlier.shape = NA) +
...
# test if adjusted p-value  distributions significantly different with Mann-Whitney test (Wilcoxon rank sum test)
res <- wilcox.test(x = gsea.genes[same.efo == FALSE, -log10(p.adjusted)], y = gsea.genes[same.efo == TRUE, -log10(p.adjusted)])
print(res)
print(res$p.value)

# export
fwrite(gsea.genes[p.adjusted < 0.05, ], "../dat/gsea.genes.tsv", sep = "\t")

### question 2: are genes differentially expressed after treatment with drug for disease X enriched for genes genetically associated with disease X?

## perform Fisher's test
# loop through diseases
fisher.drugs <- foreach (i = seq(opentargets.drugs.list), .combine = rbind, .errorhandling = "remove") %do% {
    ...
    foreach (j = 1:nrow(opentargets.drugs.list[[i]]), .combine = rbind, .errorhandling = "remove") %dopar% {
             ...
             if (length(degs) > 0) {
                # test significance
                tmp <- checkOverlapSignificance(degs, stopgap.list[[efo.id]]$ensembl.id, ensembl.ids)
                # annotate
                tmp <- cbind(chembl.id = this.chembl.id, efo.id, tmp)
                tmp <- merge(unique(opentargets.drugs[, .(efo.id, efo.term, chembl.id, chembl.name)]), tmp)
                setcolorder(tmp, c("chembl.id", "chembl.name", "efo.id", "efo.term", "set1", "set2", "overlap", "universe", "odds.ratio", "p.value"))
             }
    }
}
...
# export
fwrite(fisher.drugs[p.adjusted < 0.05, ], "../dat/fisher.drugs.tsv", sep = "\t")
...
# export
fwrite(gsea.drugs[p.adjusted < 0.05, ], "../dat/gsea.drugs.tsv", sep = "\t")
